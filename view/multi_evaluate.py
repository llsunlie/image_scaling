...
def evaluate_v(SRC_PATH, ORIGINAL_IMG_PATH, TARGET_DIR, ALGORITHM_NAME_LIST, ALGORITHM_ARGS_MAP, SHOW_LOG=True, SHOW_IMG=True, SHOW_BAR=True):
    # 被缩放的图像路径
    ...
    # 各种缩放算法处理后的图像路径
    TARGET_IMG_RESOLUTION_RATIO = ORIGINAL_IMG_NAME.split(
        '_')[-1].split('.')[0]
    if SHOW_LOG:
    ...
    for algorithm_name in ALGORITHM_NAME_LIST:
        if algorithm_name in ALGORITHM_ARGS_MAP.keys():
            ...
            target_path = TARGET_DIR + IMG_META_NAME + '_' + IMG_RESOLUTION_RATIO + '/' + algorithm_name + \
                '/' + IMG_META_NAME + '_' + TARGET_IMG_RESOLUTION_RATIO + arg_name + IMG_SUFFIX
        else:
            target_path = TARGET_DIR + IMG_META_NAME + '_' + IMG_RESOLUTION_RATIO + '/' + \
                algorithm_name + '/' + IMG_META_NAME + '_' + \
                TARGET_IMG_RESOLUTION_RATIO + IMG_SUFFIX
...
def evaluate_h(SRC_PATH, ORIGINAL_IMG_PATH, TARGET_DIR, ALGORITHM_NAME_LIST, ALGORITHM_ARGS_MAP, SHOW_LOG=True, SHOW_IMG=True, SHOW_BAR=True):
    # 被缩放的图像路径
    IMG_NAME = SRC_PATH.split('/')[-1]
    IMG_SUFFIX = '.' + IMG_NAME.split('.')[-1]
    IMG_RESOLUTION_RATIO = IMG_NAME.split('_')[-1].split('.')[0]
